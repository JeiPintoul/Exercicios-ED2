...
public class BST {
    ...
    public int qtdFolhas() {
        return qtdFolhas(this.raiz);
    }

    private int qtdFolhas(No noRaiz) {
        if (noRaiz == null) {
            return 0;
        } else if (noRaiz.dir == null && noRaiz.esq == null) {
            return 1;
        } else return qtdFolhas(noRaiz.esq) + qtdFolhas(noRaiz.dir);
    }

    public boolean isBinaria() {
        return isBinaria(this.raiz);
    }

    private boolean isBinaria(No noRaiz) {
        if (noRaiz == null) {
            return true;
        }

        if (noRaiz.dir == null && noRaiz.esq == null) {
            return true;
        }

        if (noRaiz.dir != null && noRaiz.esq != null) {
            return isBinaria(noRaiz.esq) && isBinaria(noRaiz.dir);
        }
